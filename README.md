...
This is a rebuild of an earlier project, redone with a stricter process: read the reference
code, understand *why* it works before reusing it, and prove every claim with evidence
...
| [`lab-06-keyboard-pipeline`](assets/evidence/lab-06-keyboard-pipeline) | Debounce, key events, repeat, simultaneous-error policy | debugger walkthrough video |
| [`lab-07-hid-report-layer`](assets/evidence/lab-07-hid-report-layer) | Key event -> 8-byte HID report, no transport yet | debugger walkthrough video |
| [`lab-08-hid-keyboard-baseline`](assets/evidence/lab-08-hid-keyboard-baseline) | Real USB keyboard end to end, macro sequences | Wireshark + hardware demo video |
...
| [`lab-14-composite-debug-tool-gui`](assets/evidence/lab-14-composite-debug-tool-gui) | tkinter GUI: CDC log + all vendor commands + RAM dump in one window | screenshot, logs, demo video |
