"""
vendor_usb.py - USB vendor command layer for stm32g0-usb-device-lab.

All vendor commands use EP0 control IN (bmRequestType = 0xC0):
    ...
    START_RAM_DUMP     (0x04) - trigger RAM dump stream over the vendor bulk
                                IN endpoint (fixed 144 KB SRAM1 region).

Protocol reference: docs/vendor-protocol.md - this file must match it exactly.
...
FW_FEATURE_REPEAT_CONTROL = 1 << 3

# Magic word in GET_FIRMWARE_INFO response: "SG0L" little-endian
FW_INFO_MAGIC = 0x4C304753

# ── LED mode values - must match VendorLedMode_t in usb_vendor_cmd.h ──────────
LED_OFF        = 0
LED_ON         = 1
...
class VendorUsb:
    ...
    def get_firmware_info(self) -> dict | None:
        """Send GET_FIRMWARE_INFO and parse the 17-byte FirmwareInfo_t response.

        Layout (packed, little-endian) - see docs/vendor-protocol.md:
            ...
            offset 15: hidInEp            uint8
            offset 16: cdcLogInEp         uint8
        Total: 17 bytes - matches FirmwareInfo_t __attribute__((packed)) in usb_vendor_cmd.h.

        Returns a dict with decoded fields, or None on failure/bad magic.
        """
        data = self._ctrl(REQ_GET_FIRMWARE_INFO, length=17)
        ...
        hid_if, cdc_ctrl_if, cdc_data_if, hid_ep, cdc_ep = struct.unpack_from("BBBBB", data, 12)
        ...
        if features & FW_FEATURE_REPEAT_CONTROL: feature_names.append("REPEAT_CONTROL")
    ...
    def start_ram_dump(self, progress_cb=None) -> tuple[bytes, float] | None:
        """Trigger a RAM dump and read it from the vendor bulk IN endpoint.

        Step 1: EP0 START_RAM_DUMP returns a 6-byte VendorDumpResponse_t
                {status(1), request(1), acceptedLength(4)}. Returns
                status=ERROR, acceptedLength=0 if a dump is already active.
        Step 2: Read exactly acceptedLength bytes from the bulk endpoint.
                No ZLP terminates the transfer - stop after acceptedLength
                bytes (see docs/vendor-protocol.md).
        ...
        # Step 2 - read the bulk stream from the vendor bulk IN endpoint
        received = bytearray()
